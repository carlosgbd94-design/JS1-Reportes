...
function api_saveSR(payload) {
  try {
    ...
    const bcg = nonNegNumberOrZero_(payload?.bcg);
    const hepatitis_b = nonNegNumberOrZero_(payload?.hepatitis_b);
    const hexavalente = nonNegNumberOrZero_(payload?.hexavalente);
    const dpt = nonNegNumberOrZero_(payload?.dpt);
    const rotavirus = nonNegNumberOrZero_(payload?.rotavirus);
    const neumococica_13 = nonNegNumberOrZero_(payload?.neumococica_13);
    const neumococica_20 = nonNegNumberOrZero_(payload?.neumococica_20);
    const srp = nonNegNumberOrZero_(payload?.srp);
    const sr = nonNegNumberOrZero_(payload?.sr);
    const vph = nonNegNumberOrZero_(payload?.vph);
    const varicela = nonNegNumberOrZero_(payload?.varicela);
    const hepatitis_a = nonNegNumberOrZero_(payload?.hepatitis_a);
    const td = nonNegNumberOrZero_(payload?.td);
    const tdpa = nonNegNumberOrZero_(payload?.tdpa);
    const covid_19 = nonNegNumberOrZero_(payload?.covid_19);
    const influenza = nonNegNumberOrZero_(payload?.influenza);
    const vsr = nonNegNumberOrZero_(payload?.vsr);

    const sh = getSheet_(SHEET_SR_EXISTENCIA);
    ensureHeader_(sh, [
      "id","timestamp","fecha","municipio","clues","unidad",
